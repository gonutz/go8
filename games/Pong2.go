package games

var Pong2 = &Game{
	Name:        "Pong",
	Description: "For 2 Players",
	HowToPlay: //
	`A tennis game for two: player one
	uses A and Q, player two UP and DOWN
	to move the paddle on the screen.
	Miss the ball and the other
	player gets the point!`,
	BackgroundColor: Color{0, 128, 0},
	ForegroundColor: Color{255, 255, 255},
	Keys: map[GameKey]uint8{
		KeyA: 4,
		KeyQ: 1,
		Up:   12,
		Down: 13,
	},
	ScreenShot: [32]uint64{
		18446744073709551615, 3221225472, 2202252345344, 6597070290944,
		2202252345344, 2202248675328, 7699810484224, 0, 3221225472, 3221225472,
		9223372040076001280, 9223372036854775808, 9223372040076001280,
		9223372040076001280, 9223372040076001280, 9223372036854775808,
		3221225473, 3221225473, 3221225473, 1, 576460755524648961, 3221225473,
		3221225472, 0, 3221225472, 3221225472, 3221225472, 0, 3221225472,
		3221225472, 3221225472, 18446744073709551615,
	},
	Program: []byte{
		,
	},
}
